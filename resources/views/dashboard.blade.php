...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
    ...
    <div class="bg-white p-6 rounded-xl shadow-sm flex items-center transition-transform hover:-translate-y-1">
        ...
        <div>
            ...
            <p class="text-xs text-orange-500 mt-1 font-semibold">Lewat jam 07:30</p>
        </div>
    </div>
    ...
    <div class="bg-white p-6 rounded-xl shadow-sm">
        ...
        <div class="space-y-4 overflow-y-auto max-h-80">
            @forelse($result_activity as $row)
                <div class="flex items-center p-3 hover:bg-gray-50 rounded-lg transition-colors border-b border-gray-50">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-500">
                            ...
                            <span class="ml-1 px-1.5 py-0.5 rounded-full text-[10px] {{ $row->status == 'Hadir' ? 'bg-green-100 text-green-600' : 'bg-yellow-100 text-yellow-600' }}">{{ $row->status }}</span>
                        </p>
                    </div>
                </div>
            @empty
                    ...
                    <tr class="hover:bg-gray-50 transition">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center">
                                <div class="h-10 w-10 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center font-bold text-xs mr-3">{{ strtoupper(substr($row->username,0,2)) }}</div>
                                <div>
                        ...
                        <td class="px-6 py-4"><span class="text-sm text-gray-500">{{ $row->email ?? '-' }}</span></td>
                        <td class="px-6 py-4">
                            @if($row->is_active == '1')
                        ...
                        <td class="px-6 py-4"><span class="text-sm text-gray-500">{{ date('d M Y', strtotime($row->created_at)) }}</span></td>
                    </tr>
                @empty
                    <tr>
